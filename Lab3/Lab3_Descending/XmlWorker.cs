...
namespace Lab3_Descending
{
    public static class XmlWorker
    {
            ...
            out List<NonTerminal> nonTerminals, out NonTerminal start)
        {
            ...
            start = nonTerminalsT.Find(nt => nt.Name == (string)grammarXml.Root.Element("startsymbol").Attribute("name"));
            start.IsStart = true;
            ...
            {
                // Сортируем от наибольшего к меньшему количеству символов для дальнейшего удобства
                production.Value.Sort(new GenerationCountComparer());
            }
            ...
            Dictionary<NonTerminal, List<Generation>> productions)
        {
            ...
            foreach (var nonterminal in nonTerminals)
            {
                ...
                nonterminalsymbols.Add(new XElement("nonterm", new XAttribute("name", nonterminal.Name)));
            }
